...
// ── INIT ──────────────────────────────────────────────────
// ── SECURITY LOCKDOWN (browser layer) ────────────────────
(function initSecurity() {
  // Right-click
  document.addEventListener('contextmenu', e => e.preventDefault(), true);

  // Clipboard
  document.addEventListener('copy',  e => { if (!submitted) e.preventDefault(); }, true);
  document.addEventListener('cut',   e => { if (!submitted) e.preventDefault(); }, true);
  ...
  // Text selection (prevent select-all to copy questions)
  document.addEventListener('selectstart', e => {
    // Allow selection inside input fields and textareas only
    if (!['INPUT','TEXTAREA'].includes(e.target.tagName)) {
      // Allow it for highlight tool — the script.js highlight feature handles this
    }
  });

  // Electron bridge — handle focus loss from main process
  if (window.examBridge) {
    window.examBridge.onFocusLost(() => {
      // If exam is active and not already paused, show the overlay
      if (!submitted && !paused) {
        logSecurityEvent('FOCUS_LOST');
        showFocusLostOverlay();
      }
    });
    window.examBridge.onFocusRestored(() => {
      hideFocusLostOverlay();
    });
    window.examBridge.onSecurityWarning((msg) => {
      logSecurityEvent('SECURITY_WARNING', msg);
    });
  }

  // Fallback: visibility change (works in regular browser too)
  ...
  // DevTools size heuristic (fallback when not in Electron)
  if (!window.examBridge) {
    setInterval(() => {
      const threshold = 160;
      if (
        ...
        logSecurityEvent('DEVTOOLS_SUSPECTED');
      }
    }, 2000);
  }
})();

// ── Security event log (stored in localStorage) ───────────
const SECURITY_LOG_KEY = 'asb-security-log';
function logSecurityEvent(type, detail = '') {
  ...
  try {
    ...
    existing.push(entry);
    localStorage.setItem(SECURITY_LOG_KEY, JSON.stringify(existing));
  } catch (_) {}
  console.warn('[SECURITY]', type, detail);
}

// ── Focus-lost overlay (shown when window blurs in Electron) ──
function showFocusLostOverlay() {
  let overlay = document.getElementById('focusLostOverlay');
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = 'focusLostOverlay';
    ...
    `;
    overlay.innerHTML = `
      <div style="color:#fff;font-size:1.2rem;font-weight:800;font-family:Inter,sans-serif;">Exam Hidden</div>
      <div style="color:rgba(255,255,255,0.5);font-size:0.8rem;font-family:Inter,sans-serif;max-width:280px;text-align:center;line-height:1.6;">
        Return to the exam window to continue. This event has been logged.
      </div>
    `;
